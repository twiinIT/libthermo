...
namespace pythermo
{
    using PG8 = PolyIdealGas<PolyIdealGasProps<double, 8>>;
    ...
    public:
        ...
        PyPG8(const PolyIdealGasProps<double, 8>& props)
    ...
    void poly_gas(py::module_& m)
    {
        ...
        auto PolyIdealGasProps8 = py::class_<PolyIdealGasProps<double, 8>>(m, "PolyIdealGasProps8")
                                      .def(py::init<const std::array<double, 8>&, const double&, const double&>(),
                                           "cp_coeffs"_a,
                                           "h0"_a,
                                           "r"_a);
        PolyIdealGasProps8.def(py::pickle(
            [](const PolyIdealGasProps<double, 8>& p) -> py::tuple {  // __getstate__
                return py::make_tuple(p.cp_coeffs, p.h_coeffs[8], p.r);
            },
            [](py::tuple t) -> PolyIdealGasProps<double, 8>
            {  // __setstate__
                PolyIdealGasProps<double, 8> p(t[0].cast<std::array<double, 8>>(), t[1].cast<double>(), t[2].cast<double>());
                return p;
            }));

        auto polygas8 = py::class_<PyPG8, PyThermo<array_t>, std::shared_ptr<PyPG8>>(m, "PolyGas8");
        polygas8.def(py::init<const PolyIdealGasProps<double, 8>&>(), "gas_properties"_a);
        polygas8.def(py::pickle(
            [](const PyPG8& g) -> py::tuple {  // __getstate__
                return py::make_tuple(&g.properties());
            },
            [](py::tuple t) -> PyPG8 {  // __setstate__
                PyPG8 g(t[0].cast<PolyIdealGasProps<double, 8>>());
                return g;
            }));
    }
}