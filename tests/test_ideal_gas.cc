...
TEST(IdealGas, Constant) 
{
  ...
  for (auto r : constants)
  {
    ...
    ASSERT_DOUBLE_EQ(gas.template R<double>(), r);
  }
}
...
TEST(IdealGas, PR) 
{
  ...
  for(auto tau : taus)
  {
    for(auto eff_poly : eff_polys)
    {
      ...
      ASSERT_DOUBLE_EQ(gas.PR(288.15, 288.15*tau, eff_poly), std::exp(std::log(tau) * eff_poly * gas.Cp(288.15) / gas.template R<double>()));
    }
  }
}
  ...
  for(auto tau : taus)
  {
    for(auto pr : prs)
    {
      ...
      ASSERT_DOUBLE_EQ(gas.EffPoly(101325., 288.15, 101325.*pr, 288.15*tau), 
                       gas.template R<double>() * log(pr) / (gas.Phi(288.15*tau) - gas.Phi(288.15)));
    }
  }
}
