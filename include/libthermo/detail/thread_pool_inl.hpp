...
namespace thermo
{
    ...
    template <class T>
    template <typename F>
    void thread_pool<T>::run_blocks(const T first_index, const T index_after_last, F&& func, const std::size_t min_size)
    {
        ...
        if (index_after_last > first_index)
        {
            ...
            for (T i = 0; i < m_size; ++i)
            {
                if (i < blks.num_blocks())
                    p_jobs[i] = [i, func = std::forward<F>(func), start = blks.start(i), end = blks.end(i)]()
                    { func(i, start, end); };
                else
                    p_jobs[i] = nullptr;
            }
