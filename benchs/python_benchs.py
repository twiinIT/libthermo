...
def bench_numpy(g, size, repeat, number):
    N = int(size)
    t1 = np.repeat(273.0, N)
    t2 = np.repeat(500.0, N)
    p1 = np.repeat(101325.0, N)
    p2 = np.repeat(5e5, N)
    eff = np.repeat(0.8, N)
    res = np.ones_like(t1)  # noqa: F841
    ...
    bench("cp", lambda: (res := g.cp(t1)))  # noqa: F841
    bench("gamma", lambda: (res := g.gamma(t1)))  # noqa: F841
    bench("h", lambda: (res := g.h(t1)))  # noqa: F841
    bench("phi", lambda: (res := g.phi(t1)))  # noqa: F841
    bench("pr", lambda: (res := g.pr(t1, t2, eff)))  # noqa: F841
    bench("eff_poly", lambda: (res := g.eff_poly(p1, t1, p2, t2)))  # noqa: F841

    return results


def warm_up():
    t = np.repeat(273.0, 1e6)
...
# for s, n in ((1e1, 1000), (1e2, 1000), (1e3, 100), (1e4, 10000),
# (1e5, 1000), (1e6, 1000), (1e7, 100), (1e8, 10)):
for s, n in (
    (1e1, 1),
    (1e2, 1),
    (1e3, 1),
...
class NumpyRealGas:
    ...
    def dphi(self, t1, t2):
        return (
            np.polyval(self.phi_coeff[:-1], t2) - np.polyval(self.phi_coeff[:-1], t1)
        ) + self.phi_coeff[-1] * np.log(t2 / t1)
